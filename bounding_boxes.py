...
def bounding_box_dimensions(x, y):
...
# Calculate bounding box dimensions for Hockenheim corners
h_bounding_boxes = []
for i in range(len(h_indices)):
    x_corner = h_x[h_indices[i] - 10: h_indices[i] + 11]
    y_corner = h_y[h_indices[i] - 10: h_indices[i] + 11]
    width, height, aspect_ratio = bounding_box_dimensions(x_corner, y_corner)
    h_bounding_boxes.append((width, height, aspect_ratio))

    print(h_bounding_boxes[i][2])

# Calculate bounding box dimensions for Silverstone corners
s_bounding_boxes = []
for i in range(len(s_indices)):
    x_corner = s_x[s_indices[i] - 10: s_indices[i] + 11]
    y_corner = s_y[s_indices[i] - 10: s_indices[i] + 11]
    width, height, aspect_ratio = bounding_box_dimensions(x_corner, y_corner)
    s_bounding_boxes.append((width, height, aspect_ratio))

    print(s_bounding_boxes[i][2])

print(h_bounding_boxes)
print(s_bounding_boxes)

# Plot bounding boxes for Hockenheim corners
plt.figure(figsize=(12, 6))
for i, (width, height, aspect_ratio) in enumerate(h_bounding_boxes):
    plt.plot([i], [aspect_ratio], marker='o', color='blue', label=f'Turn {i+1} - Hockenheim')

# Plot bounding boxes for Silverstone corners
for i, (width, height, aspect_ratio) in enumerate(s_bounding_boxes):
    plt.plot([i], [aspect_ratio], marker='o', color='red', label=f'Turn {i+1} - Silverstone')

plt.xlabel('Corner Index')
plt.ylabel('Aspect Ratio (Width/Height)')
plt.title('Bounding Box Aspect Ratio for Track Corners')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()